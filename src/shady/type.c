...
const Type* check_type_if_instr(IrArena* arena, If if_instr) {
    if (extract_operand_type(arena, if_instr.condition->type) != bool_type(arena))
...
const Type* check_type_prim_op(IrArena* arena, PrimOp prim_op) {
    ...
    switch (prim_op.op) {
        ...
        case mod_op: {
            ...
            const Type* first_operand_type = extract_operand_type(arena, prim_op.operands.nodes[0]);
            ...
            for (size_t i = 0; i < prim_op.operands.count; i++) {
                ...
                deconstruct_operand_type(arena, arg->type, &arg_actual_type, &arg_uniform);
        ...
        case and_op: {
            ...
            const Type* first_arg_type = extract_operand_type(arena, prim_op.operands.nodes[0]->type);
            for (size_t i = 0; i < prim_op.operands.count; i++) {
                const Node* arg = prim_op.operands.nodes[i];
                is_uniform &= is_operand_uniform(arena, arg->type);
                const Type* arg_type = extract_operand_type(arena, arg->type);
        ...
        case neq_op: {
            assert(prim_op.operands.count == 2);
            bool is_result_uniform = true;
            const Type* first_op_type = extract_operand_type(arena, prim_op.operands.nodes[0]);
            ...
            for (size_t i = 0; i < prim_op.operands.count; i++) {
                ...
                bool op_uniform;
                const Type* op_type;
                deconstruct_operand_type(arena, arg->type, &op_type, &op_uniform);
        ...
        case load_op: {
            ...
            bool ptr_uniform;
            const Node* ptr_type;
            deconstruct_operand_type(arena, ptr->type, &ptr_type, &ptr_uniform);
        ...
        case lea_op: {
            ...
            bool uniform = is_operand_uniform(arena, base->type);

            const Type* curr_ptr_type = extract_operand_type(arena, curr_ptr_type);
            ...
            if (offset) {
                ...
                bool offset_uniform;
                deconstruct_operand_type(arena, offset->type, &offset_type, offset_uniform);
            ...
            while (true) {
                ...
                deconstruct_operand_type(arena, selector->type, &selector_type, &selector_uniform);
        ...
        case reinterpret_op: {
            ...
            const Type* src_type;
            bool src_uniform;
            deconstruct_operand_type(arena, source->type, &src_type, &src_uniform);
        ...
        case select_op: {
            ...
            deconstruct_operand_type(arena, prim_op.operands.nodes[0]->type, &condition_type, &condition_uniform);

            const Type* alternatives_types[2];
            bool alternatives_uniform[2];
            bool alternatives_all_uniform = true;
            for (size_t i = 0; i < 2; i++) {
                deconstruct_operand_type(arena, prim_op.operands.nodes[1 + i]->type, &alternatives_types[i], &alternatives_uniform[i]);
        ...
        case extract_op: {
            ...
            bool is_uniform;
            deconstruct_operand_type(arena, source->type, &current_type, &is_uniform);
            ...
            for (size_t i = 1; i < prim_op.operands.count; i++) {
                ...
                if (dynamic_index) {
                    ...
                    bool index_uniform;
                    deconstruct_operand_type(arena, ith_index->type, &index_type, &index_uniform);
        ...
        case convert_op: {
            ...
            const Type* src_type;
            bool is_uniform;
            deconstruct_operand_type(arena, prim_op.operands.nodes[1], &src_type, &is_uniform);
            return qualified_type(arena, (QualifiedType) {
                .is_uniform = is_uniform,
                .type = dst_type
            });
        ...
        case subgroup_broadcast_first_op: {
            ...
            return qualified_type(arena, (QualifiedType) {
                .is_uniform = true,
                .type = extract_operand_type(arena, prim_op.operands.nodes[0]->type)
            });
        }
...
const Type* check_type_call_instr(IrArena* arena, Call call) {
    ...
    bool callee_uniform;
    deconstruct_operand_type(arena, call.callee, &callee_type, &callee_uniform);
...
const Type* check_type_branch(IrArena* arena, Branch branch) {
    ...
    switch (branch.branch_mode) {
        case BrTailcall: {
            const Type* callee_type;
            bool callee_uniform;
            deconstruct_operand_type(arena, branch.target->type, &callee_type, &callee_uniform);
        ...
        case BrJump: {
            ...
            bool target_uniform;
            deconstruct_operand_type(arena, branch.target->type, &target_type, &target_uniform);
        ...
        case BrIfElse: {
            const Type* condition_type;
            bool uniform;
            deconstruct_operand_type(arena, branch.branch_condition->type, &condition_type, &uniform);
            ...
            for (size_t i = 0; i < 2; i++) {
                const Type* target_type;
                bool target_uniform;
                deconstruct_operand_type(arena, branches[i]->type, &target_type, &target_uniform);
...
const Type* check_type_join(IrArena* arena, Join join) {
    ...
    deconstruct_operand_type(arena, join.join_at->type, &join_target_type, &join_target_uniform);
...
const Type* check_type_callc(IrArena* arena, Callc callc) {
    ...
    const Type* callee_type;
    bool callee_uniform;
    deconstruct_operand_type(arena, callc.callee->type, &callee_type, &callee_uniform);
    ...
    deconstruct_operand_type(arena, callc.ret_cont->type, &ret_cont_type, &ret_cont_uniform);
